...
data_=pd.read_csv("/Users/olgabuchel/downloads/decksample/classification_project/canada/toronto/toronto_data.csv", index_col=0)#.sort_values(by=['Country/Region'])#.sort(['Country/Region'], ascending=[1])
#print(data)
#data9=data_.T
data=data_
#data9.drop(['Province/State','Country/Region', 'Lat', 'Long'])
print(data_.index)

rr=list(data_.index)#[:len(list(data.index))-1]
data0=data_
#pd.read_csv("Africa_4_8_21.csv").sort_values(by=['Country/Region'])
#data10_=pd.DataFrame()
#data10_["Country/Region"]=list(data_.columns)
#data10_["ind"]=rr
...
data["data"]=pd.to_datetime(data.index,format="%Y-%m-%d")
...
with open("fire_toronto.json", "w") as fp:
    json.dump(arrs,fp)

arr=[]
for item in list(close.keys()):
    if close[item]==1:
        arr.append(item)
for item in list(close.keys()):        
    if close[item]==2:
