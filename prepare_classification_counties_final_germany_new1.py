...
for name in counties:
    ...
    values = e_dataframe1[name].fillna(0).tolist()[90:]
    ...
    if name in recs:
        ...
        with open(output_directory + '/classification/data_counties_'+str(ids[recs.index(name)])+'.json', 'w') as outfile:
            json.dump({"dates":tim2[90:],"max_14":int(max(y5)),"max":int(max(y)),"value":y3,"time":tim[90:],"original_values": original_values},outfile)
