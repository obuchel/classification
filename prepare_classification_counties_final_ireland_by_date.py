...
for name in counties:
    ...
    if len(start) > 0:
        ...
        if max0 > 0:
            ratio = y3[-1] / max0
            recent_mean = float(np.mean(original_values[-14:]))
            #print(recent_mean,ratio,name)
            if ratio>0 and recent_mean>0:
                color = classify(ratio, recent_mean, threshold)
            elif recent_mean==0 and name in ["Limerick","Kildare","Leitrim","Roscommon","Laois","Tipperary","Clare"]:
                color = "green"
            else:
                color="yellow"
        else:
            ...
            ratio=0
            color="green"
        print(recent_mean,ratio,name,color,original_values)
        plt.title(name)
