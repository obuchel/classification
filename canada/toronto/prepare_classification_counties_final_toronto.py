...
#date_of_analysis='03/31/21'

output_directory = '../output_toronto'
...
recs = list(data.columns)        

# stage latest Canada HR-level data for later processing
latest_ca_df = data[data.index != 'Reported Date']
...
tim = list(data.index)#list(e_dataframe0.columns)
#tim.pop(0)
print(tim)
ind4 = 0
aar = []
...
for name in counties:
    ...
    if len(start) > 0:
        ...
        if max0 > 0:
            ...
            color="green"
        if name in recs:    
            with open(output_directory + '/classification/data_counties_'+str(ids[recs.index(name)].replace("/","_"))+'.json', 'w') as outfile:
