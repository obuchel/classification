...
var data3=[];    
    ...
    var data0=[];
            fetch('output_canada/classification/classification_ids_provinces2.json').then(res => res.json()) 
.then(data0 => { 
             // console.log(data0);  
         var cols={"green":0,"yellow":0.1,"orange":0.4,"red":1}       
           ...
           //console.log(ids);
        fetch('all_canada_hr.json').then(res => res.json()) 
.then(data => {  
 var data2={"type":"FeatureCollection"};
    data2["features"]=[];        
            
            for (var i=0; i<data["features"].length; i++) {
                data["features"][i]["id"]=data["features"][i]["properties"]["HR_UID"].toString();
                // if (data["features"][i]["properties"]["STATEFP"]!="72") {
                if (ids.indexOf(data["features"][i]["properties"]["HR_UID"])>-1) {
                      data["features"][i]["properties"]["c"]=data0[ids.indexOf(data["features"][i]["properties"]["HR_UID"])]["c"];
                    data["features"][i]["properties"]["max"]=data0[ids.indexOf(data["features"][i]["properties"]["HR_UID"])]["max"];
                     data["features"][i]["properties"]["max1"]=Math.log(data0[ids.indexOf(data["features"][i]["properties"]["HR_UID"])]["max"]+1)*0.5;
                data["features"][i]["properties"]["v"]=cols[data0[ids.indexOf(data["features"][i]["properties"]["HR_UID"])]["c"]];
                } else {
