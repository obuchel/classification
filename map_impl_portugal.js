...
var data3=[];    
    ...
    var data0=[];
            fetch('output_portugal_municipalities/classification/classification_ids_counties2.json').then(res => res.json()) 
.then(data0 => { 
             // console.log(data0);  
 ...
 var data2={"type":"FeatureCollection"};
    data2["features"]=[];        
            ...
            for (var i=0; i<data["features"].length; i++) {
                data["features"][i]["id"]=data["features"][i]["properties"]["Dico"].toString();
                // if (data["features"][i]["properties"]["STATEFP"]!="72") {
                if (ids.indexOf(data["features"][i]["properties"]["Dico"])>-1) {
                      data["features"][i]["properties"]["c"]=data0[ids.indexOf(data["features"][i]["properties"]["Dico"])]["c"];
                    data["features"][i]["properties"]["max"]=data0[ids.indexOf(data["features"][i]["properties"]["Dico"])]["max"];
                     data["features"][i]["properties"]["max1"]=Math.log(data0[ids.indexOf(data["features"][i]["properties"]["Dico"])]["max"]+1)*0.5;
                    data["features"][i]["properties"]["v"]=cols[data0[ids.indexOf(data["features"][i]["properties"]["Dico"])]["c"]];
		    /*
